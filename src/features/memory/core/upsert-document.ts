...
export async function upsertDocument(input: {
  ...
  if (existing) {
    ...
    const updated: MemoryRecord = {
      ...
      reviewDecisions: payload.reviewDecisions ?? existing.reviewDecisions,
      verifiedAt: payload.verifiedAt ?? existing.verifiedAt,
      verificationEvidence: payload.verificationEvidence ?? existing.verificationEvidence,
  ...
  return rememberMemory({
    payload: {
      ...
      kind: "doc",
      scope,
      verificationStatus: payload.verificationStatus ?? "hypothesis",
      reviewStatus: payload.reviewStatus ?? (payload.verificationStatus === "verified" ? undefined : "pending"),
